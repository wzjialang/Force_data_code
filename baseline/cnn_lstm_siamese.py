...
class TimeseriesNet(nn.Module):
    def __init__(self, mstcn_f_dim, out_features, len_q, device):
        super(TimeseriesNet,self).__init__()
        self.stage_1_conv = nn.Conv1d(mstcn_f_dim,64,kernel_size=5,stride=1)
    ...
    def forward(self,l):
        out_features = l.transpose(1,2).contiguous()
        inputs = []
        for i in range(out_features.size(1)):
            if i<self.len_q-1:
                input = torch.zeros((1, self.len_q-1-i, self.feature_dim)).to(self.device)
                input = torch.cat([input, out_features[:, 0:i+1]], dim=1)
            else:
                input = out_features[:, i-self.len_q+1:i+1]
            inputs.append(input)
        inputs = torch.stack(inputs, dim=0).squeeze(1).transpose(2,1).contiguous()
        inputs = F.relu(self.stage_1_conv(inputs))
        inputs = self.stage_1_pool(inputs)
        inputs = self.stage_1_drop(inputs)
        inputs = self.stage_1_norm(inputs)
        inputs = F.relu(self.stage_2_conv(inputs))
        inputs = self.stage_2_pool(inputs)
        inputs = self.stage_2_drop(inputs)
        inputs = self.stage_2_norm(inputs)
...
class TimeseriesNet_lstm(nn.Module):
    def __init__(self, mstcn_f_dim, out_features, len_q, device):
       ...
       #LSTM(INPUTFEATURES, HIDDEN_SIZE, NUM_LAYERS, BATCH_FIRST=TRUE)
        ...
        self.seq_len =60
        self.layer_dim =1
        self.lstm1 = nn.LSTM(mstcn_f_dim,512, dropout=0, num_layers=self.layer_dim,batch_first=True)
    ...
    def forward(self,l): 
        ...
        for i in range(l.size(1)):
            if i<self.len_q-1:
                input = torch.zeros((1, self.len_q-1-i, self.feature_dim)).to(self.device)
                input = torch.cat([input, l[:, 0:i+1]], dim=1)
            else:
                input = l[:, i-self.len_q+1:i+1]
            inputs.append(input)
        inputs = torch.stack(inputs, dim=0).squeeze(1) # total_frame, len_q, num_classes = 345, 30, 2048
        
        h0_l = torch.randn(self.layer_dim,inputs.size(0),512,device=self.device).requires_grad_()
        c0_l = torch.randn(self.layer_dim,inputs.size(0),512,device=self.device).requires_grad_()
        lstm,(hn,cn) = self.lstm1(inputs,(h0_l.detach(), c0_l.detach()))
        lstm = F.relu(lstm)
        
        h1_l = torch.randn(self.layer_dim,lstm.size(0),128,device=self.device).requires_grad_()
        c1_l = torch.randn(self.layer_dim,lstm.size(0),128,device=self.device).requires_grad_()
        lstm,(hn,cn) = self.lstm2(lstm,(h1_l.detach(), c1_l.detach()))
        lstm = F.relu(lstm)
        
        h2_l = torch.randn(self.layer_dim,lstm.size(0),64,device=self.device).requires_grad_()
        c2_l = torch.randn(self.layer_dim,lstm.size(0),64,device=self.device).requires_grad_()
...
class TimeseriesNet_dural_lstm(nn.Module):
    ...
    def forward(self,l,l1):
        ...
        h0_l = torch.randn(self.layer_dim,l.size(0),512,device=self.device).requires_grad_()
        c0_l = torch.randn(self.layer_dim,l.size(0),512,device=self.device).requires_grad_()
        ...
        h0_l1 = torch.randn(self.layer_dim,l.size(0),512,device=self.device).requires_grad_()
        c0_l1 = torch.randn(self.layer_dim,l.size(0),512,device=self.device).requires_grad_()
        lstm1,(hn1,cn1) = self.lstm1(l1,(h0_l1.detach(), c0_l1.detach()))
        lstm1 = F.relu(lstm1)
        
        h1_l1 = torch.randn(self.layer_dim,lstm.size(0),128,device=self.device).requires_grad_()
        c1_l1 = torch.randn(self.layer_dim,lstm.size(0),128,device=self.device).requires_grad_()
        lstm1,(hn1,cn1) = self.lstm2(lstm1,(h1_l1.detach(), c1_l1.detach()))
        lstm1 = F.relu(lstm1)
        
        h2_l1 = torch.randn(self.layer_dim,lstm.size(0),64,device=self.device).requires_grad_()
        c2_l1 = torch.randn(self.layer_dim,lstm.size(0),64,device=self.device).requires_grad_()
